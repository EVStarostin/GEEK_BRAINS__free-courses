...
class Cart {
    ...
    async render() {
        ...
        $('.cart .status').text(`(${cartData.length} шт. — ${cartData.reduce((sum, product) => {return sum + +product.price}, 0)} руб.)`);
...
let cart;
window.onload = () => {
    cart = new Cart('http://89.108.65.123:8080', document.querySelector('.cart .items'));
